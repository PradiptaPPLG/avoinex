...
private function getAirlineName($code)
{
    ...
    public function search(Request $request)
    {
        ...
        $flights = FlightInstance::with([
            ...
            ->whereHas('schedule', function($query) use ($originIataCodes, $destinationIataCodes) {
                $query->whereIn('origin_iata_code', $originIataCodes)
                      ->whereIn('destination_iata_code', $destinationIataCodes);
            })
            ->where('flight_date', $departDate)
            ->where('flight_status_id', 1) // Scheduled
            ->orderBy('schedule.departure_time_gmt')
            ->get();
