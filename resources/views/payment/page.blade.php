    ...
    <div class="row">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('payment.process') }}" method="POST">
                        ...
                        @php $exchangeRate = config('app.usd_to_idr', 15000); @endphp
                        <button type="submit" class="btn btn-lg w-100 fw-bold border-0" style="background-color: #FFCB2A; color: #000;">
                            <i class="bi bi-lock me-2"></i> Pay Now Rp {{ number_format($booking->total_price_usd * $exchangeRate, 0, ',', '.') }}
                        </button>
                    </form>
                </div>
            </div>
        ...
        <div class="col-md-4">
            <div class="card sticky-top" style="top: 88px; z-index: 10;">
                ...
                <div class="card-body">
                    <h6 class="mb-1">Booking #{{ $booking->booking_code ?? 'PENDING' }}</h6>
                        ...
                        <tbody>
                            ...
                            <tr style="border-bottom: 1px solid #f0f2f5;">
                                <td class="border-0 py-2">
                                    ...
                                    @if($seat->meal)
                                    <div class="text-muted small" style="margin-left: 16px;">
                                        <i class="bi bi-cup-hot" style="font-size: 10px;"></i> {{ $seat->meal->name }}
                                    </div>
                                    @endif
                                ...
                                <td class="text-end border-0 py-2 align-top">
                                    <div class="fw-bold">Rp {{ number_format($seat->price_at_booking * $exchangeRate, 0, ',', '.') }}</div>
                                    @if($seat->meal_price > 0)
                                    <div class="text-muted small">+Rp {{ number_format($seat->meal_price * $exchangeRate, 0, ',', '.') }}</div>
                                    @endif
                                    @if($seat->insurance_price > 0)
                                    <div class="text-muted small">+Rp {{ number_format($seat->insurance_price * $exchangeRate, 0, ',', '.') }}</div>
                                    @endif
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td class="border-0 py-1 text-muted" style="font-size: 11px; padding-left: 16px !important;">
                                    <i class="bi bi-suitcase" style="font-size: 10px;"></i> Baggage {{ $seat->baggage_weight }}kg
                                </td>
                                <td class="text-end border-0 py-1 text-muted" style="font-size: 11px;">+Rp {{ number_format($seat->baggage_price * $exchangeRate, 0, ',', '.') }}</td>
                            </tr>
                            @endif
                            @endforeach
                        </tbody>
                        ...
                        <tbody id="payment-fee-breakdown">
                            <tr>
                                ...
                                <td class="text-end py-1 text-muted" style="font-size: 12px; border-top: 1px dashed #dee2e6;">
                                    Rp {{ number_format($taxAmount * $exchangeRate, 0, ',', '.') }}
                                </td>
                            </tr>
                            <tr>
                        ...
                        <tfoot>
                            <tr style="border-top: 2px solid #dee2e6;">
                                ...
                                <th class="text-end py-2" style="font-size: 15px; color: #0066CC;">
                                    Rp {{ number_format($grandTotalIdr, 0, ',', '.') }}
                                </th>
                            </tr>
                        </tfoot>
                    </table>
